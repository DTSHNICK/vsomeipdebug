...
configuration_impl::load_routing(const configuration_element &_element) {
    try {
        ...
        VSOMEIP_INFO << "print_all_values(its_routing);\n";
        print_all_values(its_routing);
...
void print_all_values(const boost::property_tree::ptree& pt) {
  for (const auto& child : pt) {
    VSOMEIP_INFO << child.first << ": ";
    if (child.second.empty()) {
      // Print leaf node value
      VSOMEIP_INFO << child.second.data() << std::endl;
    } else {
...
configuration_impl::load_routing_host(const boost::property_tree::ptree &_tree,
    ...
    try {
        ...
        uid_t its_uid;
        gid_t its_gid;
        VSOMEIP_INFO << __func__ << ": _tree.size()" << _tree.size() << "\n";
